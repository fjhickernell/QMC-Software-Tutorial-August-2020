...
\thanksnote{with the GAIL an QMCPy teams \\
...
\tabularnewline
$\vT_i$ do not know about each other &
$\vX_i$ represent $F$ well
\end{tabular}
\end{frame}
...
\begin{frame}{Why Is LD Better than IID?}
	\begin{description}[<+->]
		...
		\begin{equation*}
		\mu = \Ex[f(\vX)] = \int_\Omega f(\vx) \, \varrho(\vx) \, \dif \vx \approx \hmu = \frac 1n \sum_{i=1}^n f(\vx_i)
		\end{equation*}
		LD points give faster convergence than IID
		
		\item[Design of Computer Experiments] LD can be more space filling (even than Latin hypercube sampling)

		\item[Global Optimization]  LD points can be more space filling
