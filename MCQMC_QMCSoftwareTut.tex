...
\begin{frame}{Why LD is better than IID}
	...
	\begin{description}
		...
		\begin{equation*}
		\mu := \Ex[f(\vX)] = \int_\cx f(\vx) \, \varrho(\vx) \, \dif \vx \approx \frac 1n \sum_{i=1}^n f(\vX_i) =:  \hmu
		\end{equation*}
		LD points give faster convergence than IID
		
		\item[Design of Computer Experiments] LD are more space filling (even than Latin hypercube sampling) for use in constructing surrogate models
...
\begin{frame}{What software components do we need?}
	...
	\[
	 \uncover<2->{\mu :=  \int_{\ct} \alert<3>{g(\vt)} \, \alert<2>{\lambda(\vt) \, \dif \vt} = \cdots = } \only<1>{\mu :=} 	\underbrace{\Ex[f(\vX)]}_{\text{expectation}} = \underbrace{\int_\cx \alert<3>{f(\vx)} \, \varrho(\vx) \, \dif \vx}_{\text{integration}} \approx  \frac 1n \sum_{i=1}^{\alert<4>{n}} f(\alert<1>{\vX_i}) =: \hmu
	\]
	...
	\begin{description}[<+->]
		...
		\item[Stopping Criterion] that determines how large $n$ needs to be to ensure that $\abs{\mu - \hmu} \le \varepsilon$
	\end{description}
\end{frame}
...
\begin{frame}{The argument for community software}
	...
	\item<+-> Provides a consistent interface for  different pieces from different places
	
	\item<+-> Enables reproducible computational research
	
	\item<+-> Tedious stuff only needs to be done once
