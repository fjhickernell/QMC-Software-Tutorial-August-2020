...
The LD sequences implemented as \text{DiscreteDistribution} objects typically mimic the $\calU[0,1]^d$ distribution.  However, we may need sequences to mimic other distributions.  This can be implemented by via variable transformations.  In general, if $\bsX \sim \calU[0,1]^d$, then
\begin{gather*}
\bsT = (\bsb - \bsa) \odot \bsX + \bsa \sim  \calU[\bsa,\bsb], \\
\bsT = \mA^T\bigl(\Phi^{-1}(X_1),  \ldots, \Phi^{-1}(X_d)\bigr)^T + \bsa \sim \calN(\bsa, \mSigma), \quad \text{where }  \mSigma = \mA \mA^T,
\end{gather*}
and $\odot$ denotes term-by-term (Hadamard) multiplication.  Here, $\bsa$ and $\bsb$ are assumed to be finite.  Again we use ``$\sim$'' to denote mimicry, not necessarily in a probabilistic sense.
...
\begin{align}
	\nonumber 
 \int_\calT g(\bst) \, \lambda(\bst) \, \D \bst &= \int_\calX g\bigl(\bsPsi(\bsx)\bigr) \, \lambda\bigl(\bsPsi(\bsx)\bigr) \,\QMCPYabs{\frac{\partial \bsPsi}{\partial \bsx}} \, \D \bsx = \int_\calX g\bigl(\bsPsi(\bsx)\bigr) \QMCPYabs{\frac{\partial \bsPsi}{\partial \bsx}} \, \D \bsx \\
  & = \int_\calX f(\bsx) \, \varrho(\bsx) \, \D \bsx , \quad \text{where } f(\bsx)  = g\bigl(\bsPsi(\bsx)\bigr) \QMCPYabs{\frac{\partial \bsPsi}{\partial \bsx}} \frac{\lambda(\bsPsi(\bsx))}{\varrho(\bsx)}.
\end{align}
