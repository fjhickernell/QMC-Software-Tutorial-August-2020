...
$$g(\bsx) = \max\left(\frac{1}{2d}\sum_{j=1}^d [S(\bst_{j-1},\bsx_{j-1})+S(\bst_j,\bsx_j)]-K,0\right).$$
...
%	\quad
	...
	\caption{Left: Brownian Motion paths with drift $a=2$.\\ Right: Corresponding Asian Call option paths with $S_0=35$, $K=40$. \AGSComment{Fix above figs to match what I have in presentation.}}
	\label{fig:aco}
\end{figure}
