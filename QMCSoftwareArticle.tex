...
\lstinputlisting{ags/python/qmcpy_install.py} 
...
\lstinputlisting[style=Python]{ags/python/qmcpy_import.py}
...
We illustrate the difference between lattice and Sobol' sequences using QMCPy. First, we create an instance of a $d=2$ dimensional \texttt{Lattice} object of the  \texttt{DiscreteDistribution} abstract class.  Then we generate the first eight (non-randomized) points in this lattice: \AGSComment{How to handle cut off code?}
...
\lstinputlisting[style=Python]{ags/python/first_uniform.py}
...
\lstinputlisting[style=Python]{ags/python/first_gaussian.py}
...
Figure \ref{fig:ikc} shows visualizes sampling on the original integrand versus sampling on the transformed integrand. Note that thee Keister function has been also implemented as a concrete integrand class in the QMCPy library. 
\begin{figure}
