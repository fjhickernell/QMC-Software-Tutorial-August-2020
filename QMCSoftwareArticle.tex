...
The figures in this paper can be reproduced in the Google Colab notebook \cite{QMCPyTutColab2020_paper} \AGSComment{Citation looks ok? Put this earlier?}.
...
The Brownian motion distribution arises often in financial risk applications.  Here the $d$ components of the variable $\bsT$ correspond to the Brownian motion at times $\tau/d, 2\tau/d, \ldots, \tau$, where $\tau$ is the time horizon.  The distribution is a special case of the Gaussian with covariance $\mSigma = (\tau/d) \bigl (\min(j,k) \bigr)_{j,k=1}^d$ and mean $\bsa$, which  correspond to a drift coefficient times $(\tau/d)(1, 2, \ldots, d)^T$.  \AGSComment{Talk about how $\lambda \sim \calN(0,\Sigma)$ but you can \texttt{set\_transform} corresponding to $\calN(\bsa,\Sigma)$, essentially importance sampling?} The code for generating a Brownian motion is
...
\begin{figure}
	\includegraphics[width=.45\textwidth]{QMCSoftwareArticle/figs/tm_bm.png} 
	%	\quad
	\includegraphics[width=.45\textwidth]{QMCSoftwareArticle/figs/tm_bm_drift.png}
...
\begin{itemize}
    ...
    \item PCA and Cholesky decomp for Gaussian + Brownian Motion
