...
\end{equation}
where $\varrho$ is a probability density. 
...
\begin{equation} \label{eq:samplemean}
	\hmu = \frac 1n \sum_{i=1}^n f(\bsX_i), \qquad \bsX_1, \bsX_2, \ldots \sim \varrho.
\end{equation}
Here the notation ``$\sim$'' means that the sequence mimics the specified distribution/density, but not necessarily in a probabilistic way.  The choice of this sequence, and the choice of $n$ to satisfy  the prescribed error tolerance are important decisions.  QMC software helps the user make those decisions.
...
\end{figure}
The IID points are random. The position of each point is not influenced by the other.  When we say that $\bsX_1, \bsX_2, \ldots \IIDsim \calU[0,1]^d$, we mean that for any positive integer $n$, the  multivariate probability distribution of $\bsX_1, \ldots, \bsX_n$ is the product of the marginals, specifically,
\begin{equation*}
