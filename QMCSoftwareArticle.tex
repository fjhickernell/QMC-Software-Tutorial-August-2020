...
\begin{equation}
	\abs{\mu - \hmu} = \abs{\int_{\calX} f(\bsx) \, \D (F - F_{\{\bsX_i\}_{i=0}^{n-1}}) (\bsx)} \le D(\{\bsX_i\}_{i=0}^{n-1}, F) V(f),
\end{equation} 
...
\begin{array}{cc}
	\topline
	\text{Integration Lattices} & \text{Digital Nets} \hline
	\multicolumn{2}{c}{\text{unshifted} \bsZ_i \oplus \bsZ_i' }
\end{array}
\]
