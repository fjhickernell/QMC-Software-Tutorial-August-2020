...
The LD sequences implemented as \text{DiscreteDistribution} objects typically mimic the $\calU[0,1]^d$ distribution.  However, we may need sequences to mimic other distributions.  This can be implemented by via variable transformations, $\bsPsi$.  In general, if $\bsX \sim \calU[0,1]^d$, then
...
The code below shows sets up an \texttt{Integrand} instance using QMCPy's \texttt{CustomFun} wrapper to tie a user-defined function $g$ into the QMCPy framework.  Then we evaluate the sample mean of $n=1000$ $f$ values obtained by sampling at transformed Halton points.
...
Note that the Keister function has been also implemented as a concrete integrand class in the QMCPy library.  \FJHComment{Does the built in Keister function work like the Asian options does, meaning that the problem is the same no matter the distribution? } \AGSComment{The Keister function now works with either $\calN(0,1/2)$ or the Lebesgue measure (see \texttt{integrand/code/keister} in our Colab notebook). Our Asian Option (not Mike Gile's multi-level) will only work with a Brownian Motion true measure, but will perform importance sampling automatically if drift $\neq 0$. Also, Brownian Motion now supports exercise time $\neq 1$ (see \texttt{true measure/figs/brownian motion} in our Colab notebook.}
