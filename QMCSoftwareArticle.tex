...
This tutorial points to some of the best QMC software available.  Moreover, we describe QMCPy \cite{QMCPy2020a}, which is designed to become a community owned Python library that combines the best QMC algorithms under a common user interface.
...
\AGSComment{\texttt{Keister} uses Lebesgue $\lambda$ and defaults $\bsPsi$ to \eqref{eq:exampleVarTransGauss} with $A=\sqrt{1/2}\bsI$. A user can manually set $\bsPsi$ via the \texttt{set\_transform} method, as done in the code below. Resetting $\bsPsi$ performs  \emph{importance sampling} and thus leaves $\mu$ unchanged.}  
