...
The Brownian motion distribution arises often in financial risk applications.  Here the $d$ components of the variable $\bsT$ correspond to the Brownian motion at times $\tau/d, 2\tau/d, \ldots, \tau$, where $\tau$ is the time horizon.  The distribution is a special case of the Gaussian with covariance $\mSigma = (\tau/d) \bigl (\min(j,k) \bigr)_{j,k=1}^d$ and mean $\bsa$, which  correspond to a drift coefficient times $(\tau/d)(1, 2, \ldots, d)^T$.  \AGSComment{Talk about how $\lambda \sim \calN(0,\Sigma)$ but you can \texttt{set\_transform} corresponding to $\calN(\ba,\Sigma)$, essentially importance sampling?} The code for generating a Brownian motion is
...
$$ \det(J) = \det\left(A^T . \text{diag}\left(\frac{1}{\phi(\Phi^{-1}(\bsx))}\right)\right) = \frac{\det(A^T)}{\prod_{j=1}^d \phi(\Phi^{-1}(x_j))}$$}
