...
\end{equation}
The choice of the sequence $\{\bsX_i\}_{i=0}^\infty$ and the choice of $n$ to satisfy  the prescribed error requirement,
...
\begin{equation}
	\QMCPYabs{\mu - \hmu} = \QMCPYbiggabs{\int_{\cube} f(\bsx) \, \D \bigl(F_\unif - F_{\{\bsX_i\}_{i=0}^{n-1}}\bigr) (\bsx)} \le D\bigl(\{\bsX_i\}_{i=0}^{n-1}\bigr) V(f).
\end{equation} 
...
\begin{description}[format=\textup,format=\textbf]
	...
	\item[Discrete Distribution]  produces the sequence $\bsX_0, \bsX_1, \dots$ that mimics $\calU[0,1]^d$;
...
\end{equation}
and mean $\bsa$, which  is proportional to the times $(\tau/d)(1, 2, \ldots, d)^T$. The code for generating a Brownian motion is
...
We close with an invitation.  Try QMCPy.  If you find bugs or missing features, please submit an issue to \href{https://github.com/QMCSoftware/QMCSoftware/issues}{\nolinkurl{https://github.com/QMCSoftware/QMCSoftware/issues}}.  If you wish to add your great algorithm or use case, please submit a pull request to our GitHub repository at \href{https://github.com/QMCSoftware/QMCSoftware/pulls}{\nolinkurl{https://github.com/QMCSoftware/QMCSoftware/pulls}}.  We hope that the community will embrace QMCPy.
