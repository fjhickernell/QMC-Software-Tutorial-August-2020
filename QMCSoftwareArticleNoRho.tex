...
The table below displays the variable transformations and the measures for these three cases.  In all cases $\varrho_1(\bsx) = \cdots \varrho_L(\bsx) = 1$ because $\bsPsi$ utilizes an inverse distribution.

The third integrand, \texttt{K\_gauss\_kuma}, sets $\bsPsi_2$ as \eqref{eq:exampleVarTransGauss} with $\mA = \mI$ and $\bsPsi_1(\bsx) = \bsPhi^{-1}(\bsx)$ where $\bsPhi$ is the element-wise CDF of the Kumaraswamy measure. 
