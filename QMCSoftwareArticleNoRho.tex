...
QMCPy includes support for composed variable transformations. Suppose we are given weight $\lambda$ supported on $\calT$, sampling PDF $\varrho$ supported on  $\calX$, and importance sampling transformation $\bsPsi_\IMP=\hat{\bsPsi}_L = \bsPsi_L \circ \bsPsi_{L-1} \circ \dots \circ \bsPsi_1$ as in \eqref{eq:transVarImp}. Here, $\bsPsi_l:\calX_{l-1} \to \calX_l$ transforms samples mimicking density $\varrho_l$ to mimic density $\lambda_l$, meaning $\lambda_l(\bsPsi_l(\bsx))\lvert \partial \bsPsi_l / \partial \bsx \rvert = \varrho_l(\bsx)$. We enforce that $\calX_0=\calX$ and $\calX_L=\calT$ so the transformations are compatible with the discrete distribution and true measure respectively. Let $\hat{\bsPsi}_l=(\bsPsi_l \circ \bsPsi_{l-1} \circ \dots \circ \bsPsi_1)$ be the composition of the first $l$ transforms and assume $\hat{\bsPsi}_0(\bsx)=\bsx$, the identity transform. If $\lvert \bsPsi_l'(\bsx) \rvert$ denotes the Jacobian of the $l^{\text{th}}$ transform evaluated at $\bsx$, then we may write $\mu =  \int_{\calX}f(\bsx) \, \varrho(\bsx) \D \bsx$ for 
\begin{align*}
 f(\bsx) 
 &= g\left(\hat{\bsPsi}_L(\bsx)\right)\frac{\lambda\left(\hat{\bsPsi}_L(\bsx)\right)}{\varrho(\bsx)}\prod_{l=1}^L \left\lvert \bsPsi_l'\left(\hat{\bsPsi}_{l-1}(\bsx)\right) \right\rvert \D \bsx \\
 &= g\left(\hat{\bsPsi}_L(\bsx)\right)\frac{\lambda\left(\hat{\bsPsi}_L(\bsx)\right)}{\varrho(\bsx)}\prod_{l=1}^L \frac{\varrho_l(\hat{\bsPsi}_{l-1}(\bsx))}{\lambda_l(\hat{\bsPsi}_l(\bsx))} \D \bsx.
\end{align*}

For an example, we return to the Keister function \eqref{eq:KeisterIntegral}. The following code constructs three instances: the default Keister function without importance sampling (\texttt{K}), the Keister function importance sampled by a Gaussian $\calN(0,3/4)$ measure (\texttt{K\_gauss}), and the Keister function importance sampled by both a standard Gaussian and Kumaraswamy distribution (\texttt{K\_gauss\_kuma}). The multiple importance sampling in the third integrand is valid as the Kumaraswamy transform takes the unit cube to the unite cube, and the Gaussian transform is as defined in Equation \ref{eq:exampleVarTransGauss}. The code concludes by printing the time required for QMC integration to achieve absolute tolerance $5*10^{-8}$.
...
\end{figure} \AGSComment{$\varrho_l$ and $\lambda_l$ do not need to be densities, but I think it makes sense to say they are as we think of $\bsPsi_l$ as transforming samples that have some density. Also, all of our current transforms correspond to $\varrho_l$ being a density (and therefore $\lambda_l$ as well).}
